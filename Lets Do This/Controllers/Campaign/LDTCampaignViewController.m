...
- (void)viewDidLoad {
    ...
    [self styleView];
    // Grab 50 records for now until we add paginated requests.
    NSString *galleryUrl = [NSString stringWithFormat:@"%@reportback-items?load_user=true&status=approved,promoted&campaigns=%li&count=50", [DSOAPI sharedInstance].phoenixApiURL, (long)self.campaign.campaignID];
